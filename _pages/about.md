---
permalink: /
title: ""
excerpt: "Thomas C. Martinez is a mathematics PhD candidate at UCLA working in algebraic and enumerative combinatorics."
author_profile: true
redirect_from: 
  - /about/
  - /about.html
---

<section class="home-intro">
  <p class="eyebrow">Algebraic &amp; enumerative combinatorics</p>
  <h1>Thomas C. Martinez</h1>
  <p class="home-intro__lead">I am a mathematics PhD candidate at UCLA, advised by <a href="https://www.math.ucla.edu/~galashin/">Pavel Galashin</a>. I study the combinatorics and geometry of positroid varieties, with an emphasis on affine Deodhar diagrams, Richardson links, plabic graphs, and Catalan combinatorics.</p>
  <div class="button-row">
    <a class="site-button" href="/files/TMartinezCV.pdf">Download CV</a>
    <a class="site-button site-button--secondary" href="/publications/">View research</a>
  </div>
</section>

<aside class="job-market-note" aria-label="Job market status">
  <span class="status-dot" aria-hidden="true"></span>
  <div><strong>On the academic job market.</strong> My PhD is expected in 2027.</div>
</aside>

<section class="content-section featured-section">
  <div class="section-heading section-heading--split">
    <div>
      <p class="eyebrow">Selected work</p>
      <h2>Recent research</h2>
    </div>
    <a class="text-link" href="/publications/">All publications <span aria-hidden="true">→</span></a>
  </div>
  <div class="featured-list">
    <article class="featured-item">
      <div>
        <p class="item-meta">Preprint · 2026</p>
        <h3>The Combinatorics of Affine Deodhar Diagrams</h3>
        <p>Affine diagrammatics, local moves, and rational Dyck paths.</p>
      </div>
      <div class="featured-actions">
        <a class="paper-link" href="https://arxiv.org/abs/2607.15672">arXiv</a>
        <a class="paper-link" href="/files/AffineDeodharDiagramsSlides.pdf">Slides</a>
      </div>
    </article>
    <article class="featured-item">
      <div>
        <p class="item-meta">Preprint · 2026</p>
        <h3>Unimodality of <em>q</em>-Fibonomial Coefficients for Small Cases</h3>
        <p>Joint work with undergraduate collaborators and Olha Shevchenko.</p>
      </div>
      <a class="paper-link" href="https://arxiv.org/abs/2605.12822">arXiv</a>
    </article>
  </div>
</section>

<section class="content-section about-section">
  <p class="eyebrow">Background</p>
  <h2>About me</h2>
  <p>Before UCLA, I earned a B.S. in mathematics with High Distinction and Honors from Harvey Mudd College in 2021. Alongside research, I enjoy teaching and mentoring undergraduates through UCLA's Directed Reading Program.</p>
  <p class="contact-line">The best way to reach me is at <a href="mailto:tmartinez@math.ucla.edu">tmartinez@math.ucla.edu</a>. My pronouns are he/him.</p>
</section>
